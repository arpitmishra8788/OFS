...
public class ShopkeeperDao {
    ...
    public boolean add(Shopkeeper shopkeeper) {
        ...
        try {
            ...
            if (con != null) {
                String sql = "Insert into shopkeeper(name,gender,email,contact,userid,password,role,image,RN,address) values (?,?,?,?,?,?,?,?,?,?)";
    ...
    public boolean update(Shopkeeper shopkeeper) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "update shopkeeper set name=?,gender=?,email=?,contact=?,userid=?,password=? where id=?";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setString(1, shopkeeper.getName());
                smt.setString(2, shopkeeper.getGender());
                smt.setString(3, shopkeeper.getEmail());
    ...
    public Shopkeeper getByLoginDetail(String userid, String password, String role) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from shopkeeper where userid=? and password=? and role=?";
                PreparedStatement smt = con.prepareStatement(sql);
               smt.setString(1, userid);
               smt.setString(2,password);
